...
def m4n1_1d(mat, density):
    return mn_1d_best(mat, 4, 1)

def m4n3_1d(mat, density):
    return mn_1d_best(mat, 4, 3)

def m4n2_1d(mat, density):
    return mn_1d_best(mat, 4, 2)

def m8n7_1d(mat, density):
    return mn_1d_best(mat, 8, 7)

def m8n6_1d(mat, density):
    return mn_1d_best(mat, 8, 6)
...
def create_mask(tensor, pattern="m4n2_1d", density=0.5, tensor_grad = None):
    # Reshape tensor and mask.
    
    if tensor_grad is not None:
        # tensor = tensor.abs()
        tensor = (tensor * tensor_grad).abs()
        # tensor = tensor.abs() * tensor_grad.abs()
        
        # tensor1 = tensor.abs()
        # tensor1 = (tensor * tensor_grad).abs()
    # shape = tensor1.shape
    # ttype = tensor1.type()
    shape = tensor.shape
    ttype = tensor.type()    
